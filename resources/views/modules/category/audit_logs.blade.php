            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900">📋 Category Audit Logs</h1>
                            <p class="text-gray-600 mt-1">Track all category-related activities</p>
                        </div>
                        <div class="flex space-x-3">
                            <a href="{{ route('category.v1.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">
                                ← Back to Categories
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg mb-6">
                        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                            ...
                            <div class="flex space-x-2">
                                ...
                                <a href="{{ route('category.audit.logs') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-4 py-2 rounded-lg text-sm">Clear</a>
                            </div>
                        </form>
                    </div>
                                ...
                                @forelse($logs as $log)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500">
                                            @if($log->action == 'created' && isset($log->changes['name']))
                                                Created: {{ $log->changes['name'] }} ({{ $log->changes['code'] ?? 'N/A' }})
                                            @elseif($log->action == 'updated' && isset($log->changes['new']))
                                                Updated: {{ $log->changes['new']['name'] ?? 'N/A' }}
                                            @elseif($log->action == 'deleted' && isset($log->changes['name']))
                                                Deleted: {{ $log->changes['name'] }} ({{ $log->changes['code'] ?? 'N/A' }})
                                            @else
                                                {{ $log->remarks }}
                                            @endif
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="text-gray-500">
                                                ...
                                                <p class="text-sm">No category activities recorded yet.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
