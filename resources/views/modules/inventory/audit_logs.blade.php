            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900">📋 Inventory Audit Logs</h1>
                            <p class="text-gray-600 mt-1">Track all inventory-related activities</p>
                        </div>
                        <div class="flex space-x-3">
                            <a href="{{ route('inventory.dashboard') }}" class="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg">
                                📊 Dashboard
                            </a>
                            <a href="{{ route('inventory.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">
                                ← Back to Inventory
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg mb-6">
                        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                            ...
                            <div class="flex space-x-2">
                                ...
                                <a href="{{ route('inventory.audit.logs') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-4 py-2 rounded-lg text-sm">Clear</a>
                            </div>
                        </form>
                    </div>
                                ...
                                @forelse($logs as $log)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500">
                                            @if($log->action == 'created' && isset($log->changes['item_name']))
                                                Created: {{ $log->changes['item_name'] }} - ${{ number_format($log->changes['item_price'] ?? 0, 2) }}
                                            @elseif($log->action == 'updated' && isset($log->changes['new']))
                                                Updated: {{ $log->changes['new']['item_name'] ?? 'N/A' }}
                                            @elseif($log->action == 'deleted' && isset($log->changes['item_name']))
                                                Deleted: {{ $log->changes['item_name'] }} - ${{ number_format($log->changes['item_price'] ?? 0, 2) }}
                                            @elseif(in_array($log->action, ['stock_in', 'stock_out']) && isset($log->changes['before'], $log->changes['after']))
                                                Qty: {{ $log->changes['before'] }} → {{ $log->changes['after'] }}
                                            @else
                                                {{ $log->remarks }}
                                            @endif
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="text-gray-500">
                                                ...
                                                <p class="text-sm">No inventory activities recorded yet.</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
