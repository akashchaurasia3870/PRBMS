<?php

namespace App\Services;

use App\Interfaces\BaseServiceInterface;
use App\Repositories\InventoryRepository;
use App\Models\Category;

class InventoryService implements BaseServiceInterface
{
    protected InventoryRepository $repository;

    public function __construct(InventoryRepository $repository)
    {
        $this->repository = $repository;
    }

    public function getAll()
    {
        return $this->repository->all();
    }

    public function getById(int $id)
    {
        return $this->repository->find($id);
    }

    public function store(array $data)
    {
        return $this->repository->create($data);
    }

    public function update(int $id, array $data)
    {
        return $this->repository->update($id, $data);
    }

    public function destroy(int $id)
    {
        return $this->repository->delete($id);
    }

    public function paginate(int $perPage = 10)
    {
        return $this->repository->paginate($perPage);
    }

    public function search(array $filters = [], int $perPage = 10)
    {
        return $this->repository->search($filters, $perPage);
    }

    // UI-based operations
    public function getIndexView(array $params = [])
    {
        return $this->getIndexData($params);
    }

    public function getCreateView(array $params = [])
    {
        return Category::all();
    }

    public function getEditView(int $id)
    {
        return $this->repository->find($id);
    }

    public function getDetailView(int $id)
    {
        return $this->repository->find($id);
    }

    public function submitCreateForm(array $data)
    {
        return $this->repository->create($data);
    }

    public function submitUpdateForm(int $id, array $data)
    {
        return $this->repository->update($id, $data);
    }

    public function submitDeleteForm(int $id)
    {
        return $this->repository->delete($id);
    }

    public function getIndexData(array $filters = [])
    {
        return $this->repository->getIndexData($filters);
    }

    public function getDetailData(int $id)
    {
        return $this->repository->getDetailData($id);
    }
    
    public function getDashboardData(array $filters = [])
    {
        $totalValue = $this->repository->getTotalValue($filters);
        $inventoryByCategory = $this->repository->getInventoryByCategory($filters);
        $recentItems = $this->repository->search($filters, 5);
        $lowStockItems = $this->repository->getLowStockItems(10);
        
        return [
            'total_value' => $totalValue,
            'inventory_by_category' => $inventoryByCategory,
            'recent_items' => $recentItems,
            'low_stock_items' => $lowStockItems,
            'item_count' => $this->repository->search($filters, 1)->total()
        ];
    }
    
    public function getAllForExport(array $filters = [])
    {
        return $this->repository->getAllForExport($filters);
    }
    
    public function getByIds(array $ids)
    {
        return $this->repository->getByIds($ids);
    }
    
    public function bulkDelete(array $ids)
    {
        return $this->repository->bulkDelete($ids);
    }
    
    public function getCategories()
    {
        return Category::all();
    }
    
    public function stockIn(int $id, int $quantity)
    {
        $inventory = $this->repository->find($id);
        $beforeQty = $inventory->item_qty;
        $inventory->item_qty += $quantity;
        $inventory->save();
        
        return [
            'inventory' => $inventory,
            'before_qty' => $beforeQty,
            'after_qty' => $inventory->item_qty
        ];
    }
    
    public function stockOut(int $id, int $quantity)
    {
        $inventory = $this->repository->find($id);
        
        if ($inventory->item_qty < $quantity) {
            throw new \Exception('Insufficient stock available');
        }
        
        $beforeQty = $inventory->item_qty;
        $inventory->item_qty -= $quantity;
        $inventory->save();
        
        return [
            'inventory' => $inventory,
            'before_qty' => $beforeQty,
            'after_qty' => $inventory->item_qty
        ];
    }
}